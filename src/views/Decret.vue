...
export default {
  ...
  mounted() {
    ...
    this.$http.get("legislacao.json").then((data) => {
      this.allItems = data.data.d.results.filter(item => (item?.Tipo === 'Decretos' || item?.Tipo_x0020_de_x0020_Legisla_x00e === 'Decreto' || item?.Tipo_x0020_de_x0020_Legisla_x00e === 'Decreto Lei'))
      this.items = this.allItems

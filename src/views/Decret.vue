        ...
        <div class="row">
          <div class="col-12">
            <div class="heading text-center mb-20">
              <h3 class="heading__title">Todos {{ $tc($route.meta.display) }}s</h3>
            </div>
...
export default {
  ...
  mounted() {
    ...
    this.$http.get("legislacao.json").then((data) => {
      this.allItems = data.data.d.results.filter(item => (item?.Tipo === 'Decretos' || item?.Tipo_x0020_de_x0020_Diploma === 'Decreto'))

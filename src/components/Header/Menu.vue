<template>
  <div class="collapse navbar-collapse" id="mainNavigation">
    <ul class="navbar-nav ml-auto">
      <li
        class="nav__item"
        :class="[menu.sub_menus.length ? 'has-dropdown' : '']"
      ...
      >
        <a
          ...
          :class="[
            ...
            menu.sub_menus.find((sub) => sub.to === $route.name)
              ? 'active'
              : '',
          ]"
        >
        ...
        >
          ...
          >
            ...
            <ul
              class="dropdown-menu"
              v-show="sub.sub_menus && sub.sub_menus.length"
            >
              <li
                class="nav__item"
                v-for="(sub1, index) in sub.sub_menus"
                :key="'sub1' + index"
                :class="[sub1.sub_menus.length ? 'has-dropdown' : '']"
              >
                <a
                  v-if="sub1.sub_menus.length"
                  href="#"
                  @click.prevent
                  class="dropdown-toggle nav__item-link"
                  data-toggle="dropdown"
                >
                  <span> {{ $t(sub1.name) | upper }}</span>
                </a>
    ...
    <button class="close-mobile-menu d-block d-lg-none">
      <i class="fas fa-times" @click="closeNavBar()"></i>
    </button>
  </div>
  <!-- /.navbar-collapse -->
</template>

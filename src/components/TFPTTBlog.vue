<template>
  <section class="blog-grid pb-15">
    <div class="container">
      <div class="row">
        <div class="col-sm-12 col-md-12 col-lg-6 offset-lg-3">
          <div class="heading text-center mb-40">
            <h3 class="heading__title">
              <router-link :to="{ name: 'tfptt.blog' }">
                {{ $tc('recent_article', 2) }}
              </router-link>
            </h3>
          </div>
        ...
        >
          <div class="post-item">
            <div class="post__img">
              ...
              >
                <img v-lazy="getImageUrl(article)" alt="blog image" />
              </router-link>
            </div>
            <!-- /.blog-img -->
            <div class="post__body">
              ...
              <div class="post__meta">
                <span class="post__meta-date">{{
                  article.Data_x0020_Noticia || article.Created | date
                }}</span>
              </div>
              <p
                class="post__desc"
                v-html="$options.filters.excerpt(article.Content)"
              ></p>
              <router-link
                :to="{
                  name: 'tfptt.blog-item',
...
export default {
  ...
  async mounted() {
    const data = await this.$http.get('tfpttnews.json');
